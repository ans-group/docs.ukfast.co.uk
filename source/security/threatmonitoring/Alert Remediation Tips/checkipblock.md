...
`curl --insecure -s https://git.thmon.ukfast.co.uk/threatmon-public/active-response-scripts/raw/master/check-block-status.sh | bash {IP TO CHECK}`

If you`d like to automatically remove the IP from the block list, specify `--remove-found- when using the script, like below:

`curl --insecure -s https://git.thmon.ukfast.co.uk/threatmon-public/active-response-scripts/raw/master/check-block-status.sh | bash {IP TO CHECK} --remove-found`
