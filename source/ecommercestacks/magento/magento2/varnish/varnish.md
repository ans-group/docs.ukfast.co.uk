...
By default, Varnish will not purge all static assets and will instead only purge php. This is due to the VCL only purging the assets with the X-Magento-Tags header. As this header is generated by Magento we need to adjust the /etc/varnish/default.vcl file to purge everything including static assets.

```bash
under the vcl_recv section within the purge:
