...
App.InlineSearch = (function () {
  ...
  var doSearch = function (value, syncResults, asyncResults) {
    ...
    xhr.done(function (response) {
      ...
      asyncResults(response.hits.hits.slice(0));
    });
  };
  ...
  var init = function () {
      ...
      { minLength: 3, hint: false, highlight: true },
      {
        ...
        templates: {
          ...
          suggestion: function (item) {
            ...
            var span = $('<span></span>');
            var text = item["_source"].description;
            if (text.join) {
              text = text.join("\n");
            }
            span.text(text);
    ...
    input.on('typeahead:cursorchange', function (event, suggestion) {
      input.parents('.twitter-typeahead').find('.tt-input').val(suggestion["_source"].title);
    });
  };
